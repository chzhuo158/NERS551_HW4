...
data = np.loadtxt(current_dir + "/" + reactivity_inputfile, skiprows=2)

t = data[:,0]                      # Times

rho_im = data[:,1]*beta_eff        # Inserted reactivity at that time

LAMBDA = data[:,2]                 # generation time

NORM_FACTOR = data[:,3]            # normalization factor

REL_PWR =  data[:,4]               # relative power
